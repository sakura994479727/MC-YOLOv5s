...
Usage:
    $ python path/to/models/yolo.py --cfg yolov5l.yaml
"""
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5l.yaml', ch=3, nc=None, anchors=None, pruning=False):  # model, input channels, number of classes
        super().__init__()
...
def parse_model(d, ch, pruning=False):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [CNeB, Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3STR, C3SPP, C3Ghost, CBAM, Conv_maxpool, ShuffleNetV2_InvertedResidual,
                 conv_bn_hswish, MobileNetV3_InvertedResidual, stem, MBConvBlock, DepthSepConv,GAMAttention]:
            c1, c2 = ch[f], args[0]
