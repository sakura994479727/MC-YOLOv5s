...
class C3(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, c2o, n=1, shortcut=True, g=1, e=[0.5, 0.5],
                 rate=[1.0 for _ in range(12)]):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        # c_ = int(c2 * e)  # hidden channels
...
class EPSABlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm_layer=None, conv_kernels=None,
                 conv_groups=None):
...
# -------------------------------------------------------------------------
# C3EPSA
class C3EPSA(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, c2o, n=1, shortcut=True, g=1,
                 e=[0.5, 0.5]):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        # c_ = int(c2 * e)  # hidden channels
        if isinstance(e, list):
            c1_ = int(c2o * e[0])
        ...
        self.cv2 = Conv(c1, c2_, 1, 1)
        self.cv3 = Conv(c1_ + c2_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(*[EPSABlock(c1_, c1_, shortcut, g) for i in range(n)])
        # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])
